...
//                    .filter(withId -> withId.model != null));
    ...
    public static void getEventsForKeys(Query keysQuery, Query eventModels, Consumer<Try<List<WithId<EventId, EventModel>>>> callback) {
        readEventKeys(keysQuery, (snapshot, result) -> {
            result.onSuccess(eventIds -> {
    ...
    public static void readEventKeys(Query ref, BiConsumer<DataSnapshot, Try<Stream<EventId>>> callback) {
        ref.get().addOnSuccessListener(snapshot -> callback.accept(snapshot, Try.success(Stream
                .ofAll(snapshot.getChildren())
                .map(DataSnapshot::getKey)
                .map(EventId::new))
        )).addOnFailureListener(e -> callback.accept(null, Try.failure(e)));
    }
}
